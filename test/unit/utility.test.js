...
// Converted from QUnit to Jest to address Issue #200

describe('utility tests', () => {
  test('paramCase() should convert strings to param-case', () => {
    ...
    expect(utility.paramCase('#This string has punctuation at both ends&')).toBe('this-string-has-punctuation-at-both-ends');
  ...
  test('strArg() should convert arguments into strings', () => {
  ...
  test('arrayArg() should convert arguments into arrays', () => {
    expect(utility.arrayArg('test')).toEqual(['test']);
    expect(utility.arrayArg(['test'])).toEqual(['test']);
  });

  test('trimLeftEscapeCharacters() should remove escape characters from the beginning of a string', () => {
    expect(utility.trimLeftEscapeCharacters('test')).toBe('test');
    expect(utility.trimLeftEscapeCharacters('/test')).toBe('test');
    expect(utility.trimLeftEscapeCharacters('\\test')).toBe('test');
  });

  test('compareDates() should compare dates using only the date part and ignoring time', () => {
    expect(
  ...
  test('coerceUndefined() should coerce undefined values depending on the specified type', () => {
  ...
  test('replaceTags() should replace tags in a string with control sequences', () => {
    expect(
  ...
  test('zip() should zip 2 arrays together', () => {
